...
type CronRunReportStatus = "start" | "ping" | "fail" | "stop";

// Row shape of the monitor_groups table.
...
// Row shape of the monitor_endpoints table (joined with its group name).
...
// API shape: tinyint flags become booleans, JSON columns become objects.
type EndpointResponse = Omit<
  EndpointRow,
  "is_paused" | "headers_json" | "connection_json"
> & {
  is_paused: boolean;
...
interface CronListRow extends CronRow {
  last_run_status: string | null;
  last_run_at: Date | string | null;
  last_run_error: string | null;
}

type CronResponse<T extends CronRow = CronRow> = Omit<
  T,
  "status" | "track_run" | "headers_json"
> & {
  status: boolean;
  track_run: boolean;
  headers_json: JsonObject;
...
interface CheckRunRow extends RowDataPacket {
  id: number;
  status: string;
...
interface StatsBucketRow extends RowDataPacket {
  bucket_start: string;
...
const broadcast = (type: string, payload: unknown): void => {
  const message: WsFrame = { type, payload };
  for (const client of wsClients) {
    safeSend(client, message);
  }
};

const BROADCAST_EVENTS: readonly string[] = [
  MONITOR_CHECKED_EVENT,
  GROUP_CREATED_EVENT,
  GROUP_UPDATED_EVENT,
  GROUP_DELETED_EVENT,
...
for (const eventName of BROADCAST_EVENTS) {
  monitorEvents.on(eventName, (payload: unknown) => {
...
const ALLOWED_STATS_GRANULARITIES = new Set<StatsGranularity>([
  "minute",
  "hour",
  "day",
]);
...
function toInteger(value: unknown, fallback?: number | null): number | null;
function toInteger(
  value: unknown,
...
const normalizeHttpPayload = (payload: JsonObject): JsonObject => {
  ...
  const expectedJsonPath =
    String(payload.expected_json_path ?? "").trim() || null;
  const expectedJsonValue =
    payload.expected_json_value == null ||
    String(payload.expected_json_value).trim() === ""
      ? null
  ...
  if (
    (expectedJsonPath && !expectedJsonValue) ||
    (!expectedJsonPath && expectedJsonValue)
  ) {
    throw new Error(
      "expected_json_path and expected_json_value must both be provided together",
    );
  }
...
const normalizeConnectionMonitorPayload = (
  ...
  const connection = parseJsonObjectInput(
    payload.connection_json,
    "connection_json",
  );
  const probeCommand = String(payload.probe_command ?? "").trim() || null;
  const expectedProbeValue =
    payload.expected_probe_value == null ||
    String(payload.expected_probe_value).trim() === ""
      ? null
      : String(payload.expected_probe_value);

  const defaultPortByType: Partial<Record<MonitorType, number>> = {
    mysql: 3306,
    redis: 6379,
    nats: 4222,
    tcp: 80,
...
const normalizeEndpointPayload = (payload: JsonObject): JsonObject => {
  ...
  const monitorTypeRaw = String(payload.monitor_type ?? "http")
    .trim()
    .toLowerCase();
  ...
  if (!MONITOR_TYPES.has(monitorTypeRaw)) {
    throw new Error(
      "monitor_type must be one of http, mysql, redis, nats, tcp",
    );
  }
...
const mapEndpointRow = (row: EndpointRow): EndpointResponse => {
  ...
  return {
    ...
    connection_json: maskSensitiveJson(
      parseOrDefault(row.connection_json, {}),
    ) as JsonObject,
  };
};
...
const normalizeCronPayload = (payload: JsonObject): JsonObject => {
  ...
  const triggerType = String(payload.trigger_type ?? "nats")
    .trim()
    .toLowerCase();
  const httpMethod = String(payload.http_method ?? "NONE")
    .trim()
    .toUpperCase();
  ...
  const natsSubject =
    String(payload.nats_subject ?? "").trim() || DEFAULT_CRON_NATS_SUBJECT;

  if (!cron) throw new Error("Cron name is required");
  if (cron.length > 100)
    throw new Error("Cron name must be at most 100 characters");
  if (!CRON_NAME_PATTERN.test(cron)) {
    throw new Error(
      'Cron name may only contain letters, numbers, "_", "-", "." and ":"',
    );
  }

  if (!expression) throw new Error("Cron expression is required");
  if (expression.length > 100)
    throw new Error("Cron expression must be at most 100 characters");
  ...
  if (service.length > 255)
    throw new Error("Service must be at most 255 characters");
  if (endpoint.length > 256)
    throw new Error("Endpoint must be at most 256 characters");
  ...
  return {
    ...
    body_text:
      triggerType === "http" && httpMethod === "POST" ? bodyText : null,
    nats_subject:
      triggerType === "nats" ? natsSubject : DEFAULT_CRON_NATS_SUBJECT,
...
const mapCronRow = <T extends CronRow>(row: T): CronResponse<T> => {
...
const getMappedCronByName = async (
  cronName: string,
): Promise<CronResponse | null> => {
  const [rows] = await pool.query<CronRow[]>(
    "SELECT * FROM cron_monitoring WHERE cron = ? LIMIT 1",
    [cronName],
  );
...
const parseStatsGranularity = (value: unknown): StatsGranularity => {
  const normalized = String(value ?? "hour")
    .trim()
    .toLowerCase() as StatsGranularity;
  return ALLOWED_STATS_GRANULARITIES.has(normalized) ? normalized : "hour";
};

const parseStatsRangeDays = (
  value: unknown,
...
const parseStatsMode = (value: unknown): StatsMode => {
  const normalized = String(value ?? "aggregate")
    .trim()
    .toLowerCase() as StatsMode;
...
const getMappedEndpointsByGroupId = async (
  groupId: number,
...
app.get("/api/health", async (_req: Request, res: Response) => {
  const [rows] = await pool.query<
    ({ endpoint_count: number } & RowDataPacket)[]
  >("SELECT COUNT(*) AS endpoint_count FROM monitor_endpoints");
  const [cronRows] = await pool.query<
    ({ cron_count: number } & RowDataPacket)[]
  >("SELECT COUNT(*) AS cron_count FROM cron_monitoring");
...
app.get("/api/groups", async (_req: Request, res: Response) => {
  const [rows] = await pool.query<GroupWithCountRow[]>(`
    SELECT
      g.*,
...
app.put(
  ...
  async (req: Request, res: Response) => {
    ...
    await pool.query(
      "UPDATE monitor_groups SET name = ?, description = ? WHERE id = ?",
      [name, description, id],
    );
...
app.post(
  ...
  async (req: Request, res: Response) => {
    ...
    const [groupRows] = await pool.query<IdRow[]>(
      "SELECT id FROM monitor_groups WHERE id = ?",
      [payload.group_id],
    );
    if (!groupRows.length) {
      return res.status(404).json({ error: "Group not found" });
...
app.put(
  ...
  async (req: Request, res: Response) => {
    ...
    const [existingRows] = await pool.query<
      ({ connection_json: string | JsonObject | null } & RowDataPacket)[]
    ...
    if (
      incomingBody.connection_json != null &&
      ...
      try {
        ...
        const restored = restoreMaskedJson(
          incomingConnection,
          existingConnection,
        ) as JsonObject;
...
app.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (endpoint.is_paused) {
      return res
        .status(409)
        .json({ error: "Endpoint is paused. Resume it before checking." });
    }
...
app.get("/api/endpoints/:id/runs", async (req: Request, res: Response) => {
  ...
  if (mode === "raw") {
    const [rows] = await pool.query<CheckRunRow[]>(
      `
        SELECT
          id,
          status,
  ...
  const [rows] = await pool.query<StatsBucketRow[]>(
    `
      SELECT
...
app.delete(
  ...
  async (req: Request, res: Response) => {
    ...
    const [result] = await pool.query<ResultSetHeader>(
      "DELETE FROM monitor_check_runs WHERE endpoint_id = ?",
      [id],
    );
...
app.get("/api/crons", async (_req: Request, res: Response) => {
  const [rows] = await pool.query<CronListRow[]>(`
    SELECT
      c.*,
...
app.get("/api/crons/settings", async (_req: Request, res: Response) => {
  res.json(await getCronMonitorSettings());
});
...
const requireCronRunReportAuth = (
  req: Request,
  res: Response,
  next: NextFunction,
): Response | void => {
...
const CRON_RUN_REPORT_STATUSES = new Set<string>([
  "start",
  "ping",
  "fail",
  "stop",
...
app.post(
  ...
  async (req: Request, res: Response) => {
    ...
    const status = String(req.body?.status ?? "")
      ...
      .toLowerCase();
    const errorMessage =
      req.body?.error == null ? null : String(req.body.error);

    if (!runId || runId.length > 36) {
      return res
        .status(400)
        .json({ error: "run_id is required (max 36 chars)" });
    }
    if (!cronName) {
    ...
    if (!CRON_RUN_REPORT_STATUSES.has(status)) {
      return res
        .status(400)
        .json({ error: "status must be one of start, ping, fail, stop" });
    }

    const result = await recordCronRunStatus({
      runId,
      cron: cronName,
      status: status as CronRunReportStatus,
      error: errorMessage,
    });
...
app.get("/api/crons/:cron/runs", async (req: Request, res: Response) => {
  ...
  const rangeDays =
    rangeDaysRaw == null ? null : Math.max(1, Math.min(rangeDaysRaw, 90));
  ...
  if (rangeDays != null) {
    rangeClause =
      "AND triggered_at >= DATE_SUB(UTC_TIMESTAMP(), INTERVAL ? DAY)";
...
app.post("/api/crons", requireEditor, async (req: Request, res: Response) => {
  ...
  try {
    ...
    if ((error as { code?: string } | null)?.code === "ER_DUP_ENTRY") {
      return res
        .status(409)
        .json({ error: `Cron "${payload.cron}" already exists` });
    }
    throw error;
  }
...
app.delete(
  ...
  async (req: Request, res: Response) => {
    ...
    const [result] = await pool.query<ResultSetHeader>(
      "DELETE FROM cron_monitoring WHERE cron = ?",
      [cronName],
    );
